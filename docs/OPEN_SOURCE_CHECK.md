...
| 19 | `.git/config`, remote `origin` | the GitHub personal access token used to be embedded in the remote URL, in front of the host name | Fixed. The remote is plain `https://github.com/runvouch/runvouch.git` and the token comes from the credential helper (`~/.git-credentials`, mode 600, outside the repo); it also still lives in `.env` as `GITHUB_TOKEN`. Never write a token into a remote URL: it sits in plain text in a file many tools read, a `git remote -v` in a shared terminal or log shows it, and a secret scanner reads the shape as a leaked password. Same reason the publish recipe in `packaging/github-action/PUBLISH.md` no longer uses it | done |
