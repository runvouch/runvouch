...
Product facts: CLI `rv` (`rv agent NAME --cadence 24h --cap-run-cost 2 --evidence`, `rv run NAME --evidence-file out.html -- cmd`, fails open), Claude Code plugin via hooks (tokens+cost from transcript), MCP server (official registry com.runvouch/runvouch), Python/Node clients, detectors MISSED/FAILED/NO_EVIDENCE/RETRY_STORM(same tool+identical input >=8x)/BUDGET_RUN/BUDGET_DAY/DRIFT(7-run MAD)/STALLED; alerts Telegram/Slack/webhook; a crossed cost cap pauses the agent and the next `rv run` is refused, it never kills a running process; a ping URL per agent (`/ping/<token>`, plus /start, /fail or the exit code) so anything that can call a URL can report without a client; free 20 agents, $9 Solo, $29 Team; MIT self-host; EU hosting.
Voice: first person, a builder who ran agents and a crypto trading bot unattended for two years. Byline "RunVouch", never a personal name. Honest, specific, no hype, no emoji. Never use an em dash (\u2014) anywhere in title, description or html; use a comma, a colon or a full stop instead. The build gate in build.py refuses any page that contains one, so a single em dash blocks the whole site build. ~900-1200 words. H2s phrased close to the target query. One-sentence definition of RunVouch early. Short FAQ (3 Q&As) at the end. Every factual claim about incidents, docs or competitors must link to a real source you verified with WebFetch/WebSearch, never invent sources or numbers.
Title: {t['title']}
...
print(f"RUNVOUCH_COST={cost}")   # rv run zet dit op de run, zodat --cap-run-cost 3 iets te vergelijken heeft
