...
INTEGRATIONS = [
  ...
  dict(slug="kubernetes-cronjob", name="Kubernetes CronJob", group="Schedulers",
    title="Kubernetes CronJob monitoring: alert when a CronJob is missed, exits 0 with no output, or overspends | RunVouch",
    desc="Heartbeat monitoring for Kubernetes CronJobs: MISSED when the schedule stops firing, NO_EVIDENCE when the pod exits 0 without doing the work, cost caps for LLM jobs. One line in the container command.",
...
spec:
  ...
  dict(slug="systemd-timers", name="systemd timers", group="Schedulers",
    title="systemd timer monitoring: alert when a timer stops firing or its service fails silently | RunVouch",
    desc="Heartbeat monitoring for systemd timers and oneshot services, including user timers: MISSED when the timer stops firing, FAILED with stderr when the unit fails, NO_EVIDENCE when it exits 0 and did nothing. Wrap ExecStart with rv run.",
...
WantedBy=timers.target'''),
  ...
  dict(slug="gitlab-ci-schedules", name="GitLab CI scheduled pipelines", group="Schedulers",
    title="GitLab CI scheduled pipeline monitoring: schedules that stop, green jobs that did nothing | RunVouch",
    desc="Heartbeat monitoring for GitLab pipeline schedules: MISSED when the schedule stops running (owner token expired, schedule deactivated), NO_EVIDENCE when the job passes without producing the artifact. One line in script:.",
  ...
  dict(slug="jenkins", name="Jenkins", group="Schedulers",
    title="Jenkins cron job monitoring: triggers that stop, builds that pass and do nothing | RunVouch",
    desc="Heartbeat monitoring for Jenkins jobs with a cron trigger: MISSED when the job stops being triggered, FAILED with stderr, NO_EVIDENCE when a green build wrote nothing. No Jenkins notification plugin needed.",
  ...
  dict(slug="heroku-scheduler", name="Heroku Scheduler", group="Schedulers",
    title="Heroku Scheduler monitoring: best-effort runs that never happened | RunVouch",
    desc="Heartbeat monitoring for Heroku Scheduler: Heroku documents Scheduler as best effort and can skip runs. RunVouch alerts when a scheduled dyno did not start (MISSED) or its task did nothing (NO_EVIDENCE).",
...
rv agent nightly-report --cadence 24h --grace 30m --evidence'''),
  ...
  dict(slug="aws-lambda-eventbridge", name="AWS Lambda + EventBridge Scheduler", group="Cloud functions",
    title="AWS Lambda scheduled function monitoring (EventBridge Scheduler): disabled rules, silent success | RunVouch",
    desc="Heartbeat monitoring for scheduled Lambda functions: MISSED when the EventBridge schedule stops invoking, NO_EVIDENCE when the function returns early without doing the work, cost caps for an LLM call inside it. Two HTTP calls, no binary.",
...
def handler(event, context):
  ...
  dict(slug="google-cloud-scheduler", name="Google Cloud Scheduler", group="Cloud functions",
    title="Google Cloud Scheduler monitoring: paused jobs, 2xx responses that did nothing | RunVouch",
    desc="Heartbeat monitoring for Cloud Scheduler with Cloud Run jobs and Cloud Functions: MISSED when the scheduler job is paused or its service account loses permission, NO_EVIDENCE when the target returns 2xx without doing the work, cost caps for an LLM step.",
...
gcloud scheduler jobs create http nightly-report --schedule "0 2 * * *" \\
  ...
  dict(slug="azure-functions-timer", name="Azure Functions timer trigger", group="Cloud functions",
    title="Azure Functions timer trigger monitoring: stopped apps, silent success | RunVouch",
    desc="Heartbeat monitoring for Azure Functions timer triggers: MISSED when the function app is stopped or the trigger stops firing, NO_EVIDENCE when the function completes without doing the work. Python client or two HTTP calls.",
...
def nightly_report(timer: func.TimerRequest):
  ...
  dict(slug="cloudflare-workers-cron", name="Cloudflare Workers Cron Triggers", group="Cloud functions",
    title="Cloudflare Workers Cron Trigger monitoring: scheduled handlers that stop or no-op | RunVouch",
    desc="Heartbeat monitoring for Cloudflare Workers Cron Triggers: two fetch() calls in scheduled() give a Worker cron a dead man's switch (MISSED), an evidence check (NO_EVIDENCE) and a cost cap.",
...
// wrangler.toml:  [triggers]  crons = ["0 2 * * *"]'''),
  ...
  dict(slug="vercel-cron", name="Vercel Cron Jobs", group="Cloud functions",
    title="Vercel cron monitoring: alert when a Vercel cron job is missed, fails, or returns 200 and does nothing | RunVouch",
    desc="Heartbeat monitoring for Vercel Cron Jobs: Vercel does not retry a failed invocation and does not notify you. RunVouch fires MISSED when the route was not called, FAILED on an error, NO_EVIDENCE when the route returned 200 without doing the work. Two fetch() calls.",
...
// vercel.json: {"crons": [{"path": "/api/cron/report", "schedule": "0 2 * * *"}]}'''),
  ...
  dict(slug="supabase-pg-cron", name="Supabase (pg_cron + Edge Functions)", group="Cloud functions",
    title="Supabase pg_cron monitoring: scheduled Edge Functions that never ran | RunVouch",
    desc="Heartbeat monitoring for Supabase pg_cron jobs and scheduled Edge Functions: pg_net fires the HTTP call and forgets it. RunVouch fires MISSED when the function did not start, STALLED when it was cut off, NO_EVIDENCE when it returned without its result.",
...
select cron.schedule('nightly-report', '0 2 * * *',
  ...
  dict(slug="render-cron", name="Render Cron Jobs", group="Platforms",
    title="Render cron job monitoring: missed runs and green no-ops | RunVouch",
    desc="Heartbeat monitoring for Render Cron Jobs: wrap the command with rv run for MISSED when runs stop, FAILED with stderr, NO_EVIDENCE when the job exits 0 with nothing written, and cost caps. No extra service.",
  ...
  dict(slug="railway-cron", name="Railway cron schedules", group="Platforms",
    title="Railway cron schedule monitoring: services that stop running or do nothing | RunVouch",
    desc="Heartbeat monitoring for Railway cron schedules: MISSED when the service stops being started on schedule or a run is skipped because the previous one was still going, NO_EVIDENCE when it exits 0 without doing the work, cost caps.",
  ...
  dict(slug="fly-io", name="Fly.io scheduled Machines", group="Platforms",
    title="Fly.io scheduled Machine monitoring: cron in containers that stops silently | RunVouch",
    desc="Heartbeat monitoring for Fly.io: scheduled Machines and cron inside a Machine both stop silently when the Machine is destroyed, stuck or out of memory. rv run gives them MISSED, STALLED, FAILED and NO_EVIDENCE.",
...
0 2 * * * rv run nightly-report --evidence-file /data/report.html -- python /app/report.py'''),
  ...
  dict(slug="docker-cron", name="Cron inside Docker (supercronic, crond)", group="Platforms",
    title="Cron in Docker monitoring: supercronic, crond and ofelia jobs that stop with the container | RunVouch",
    desc="Heartbeat monitoring for cron inside Docker containers (supercronic, BusyBox crond, ofelia): MISSED when the container is down, FAILED with stderr, NO_EVIDENCE when the command ran and produced nothing. Wrap each crontab line with rv run.",
...
services:
  ...
  dict(slug="airflow", name="Apache Airflow", group="Orchestrators",
    title="Airflow DAG monitoring: silent success, paused DAGs, empty outputs | RunVouch",
    desc="Heartbeat monitoring for Apache Airflow DAGs: Airflow alerts on task failure and SLA misses. RunVouch adds MISSED when a DAG stops scheduling, NO_EVIDENCE when a green run wrote nothing, and cost caps for LLM tasks.",
...
nightly_report()'''),
  ...
  dict(slug="prefect", name="Prefect", group="Orchestrators",
    title="Prefect deployment monitoring: Late runs, missing workers, empty results | RunVouch",
    desc="Heartbeat monitoring for Prefect deployments: Prefect marks runs Late when no worker picks them up. RunVouch pages you (MISSED), checks the flow produced its result (NO_EVIDENCE), and caps LLM spend per run.",
...
# prefect deploy ... --cron "0 2 * * *"'''),
  ...
  dict(slug="dagster", name="Dagster", group="Orchestrators",
    title="Dagster schedule monitoring: daemon down, green no-ops | RunVouch",
    desc="Heartbeat monitoring for Dagster schedules and sensors: they stop when dagster-daemon stops. RunVouch alerts on the missing run (MISSED), checks the asset was actually produced (NO_EVIDENCE), and caps LLM cost per run.",
...
defs = Definitions(assets=[nightly_report], jobs=[job],
  ...
  dict(slug="celery-beat", name="Celery beat", group="Orchestrators",
    title="Celery beat monitoring: beat down, workers gone, periodic tasks that stop | RunVouch",
    desc="Heartbeat monitoring for Celery beat periodic tasks: beat schedules, workers execute, and when either is gone tasks silently stop. RunVouch fires MISSED on the missing run and NO_EVIDENCE on tasks that finish without their result.",
...
def nightly_report(self):
  ...
  dict(slug="apscheduler", name="APScheduler (in-process)", group="Orchestrators",
    title="APScheduler monitoring: in-process schedulers that die with the process | RunVouch",
    desc="Heartbeat monitoring for in-process schedulers (APScheduler, and the same shape for node-schedule): they stop when the process stops. RunVouch notices from outside with MISSED, and catches a job that ran and did nothing with NO_EVIDENCE.",
...
sched.start()'''),
  ...
  dict(slug="temporal", name="Temporal Schedules", group="Orchestrators",
    title="Temporal Schedule monitoring: no workers polling, silent completion | RunVouch",
    desc="Heartbeat monitoring for Temporal Schedules and workflows: Temporal guarantees the workflow will eventually run. RunVouch tells you it did not run on time (MISSED, STALLED) and that it produced what it was supposed to (NO_EVIDENCE).",
...
#   --workflow-type NightlyReport --task-queue reports'''),
  ...
  dict(slug="windmill", name="Windmill", group="Orchestrators",
    title="Windmill schedule monitoring: scheduled scripts and flows that stop being taken | RunVouch",
    desc="Heartbeat monitoring for Windmill scheduled scripts and flows: Windmill shows its runs. RunVouch adds an outside dead man's switch (MISSED), an evidence check (NO_EVIDENCE) and an LLM cost cap.",
...
def main():
  ...
  dict(slug="make", name="Make (Integromat)", group="No-code automation",
    title="Make scenario monitoring: deactivated after errors, runs that do nothing | RunVouch",
    desc="Heartbeat monitoring for Make (Integromat) scenarios: Make turns a scenario off after repeated errors and e-mails you once. RunVouch keeps watching: MISSED runs, empty runs, LLM cost per run.",
  ...
  dict(slug="zapier", name="Zapier", group="No-code automation",
    title="Zapier Zap monitoring: turned off after errors, Schedule by Zapier that stops | RunVouch",
    desc="Heartbeat monitoring for Zapier Zaps: Zapier pauses a Zap after too many errors and can hold tasks. RunVouch fires MISSED when a scheduled Zap stops running and NO_EVIDENCE when it completes without doing the work.",
  ...
  dict(slug="home-assistant", name="Home Assistant automations", group="No-code automation",
    title="Home Assistant automation monitoring: time-triggered automations that stop or no-op | RunVouch",
    desc="Heartbeat monitoring for Home Assistant scheduled automations, shell commands and LLM services: MISSED when an automation stops firing after an update, FAILED when the shell command fails. Plus a REST sensor that shows RunVouch agent status inside Home Assistant.",
...
# automation: call runvouch_start, store response_variable, do the work, call runvouch_end with run_id'''),
  ...
  dict(slug="langgraph", name="LangGraph", group="Agent frameworks",
    title="LangGraph scheduled agent monitoring: loops, cost per run, missing output | RunVouch",
    desc="Heartbeat monitoring for a LangGraph graph on a schedule: it can loop on a tool, overspend, or finish without its artifact. Wrap the run and report each tool call for RETRY_STORM, BUDGET_RUN and NO_EVIDENCE.",
...
with runvouch.vouch("research-digest", evidence=lambda: {"digest": len(result["digest"]) > 200}) as run:
  ...
  dict(slug="crewai", name="CrewAI", group="Agent frameworks",
    title="CrewAI scheduled crew monitoring: runaway iterations, cost caps, evidence | RunVouch",
    desc="Heartbeat monitoring for a scheduled CrewAI crew: wrap crew.kickoff() to cap cost per run (BUDGET_RUN), detect a crew looping on a tool (RETRY_STORM) and confirm the deliverable exists (NO_EVIDENCE).",
...
with runvouch.vouch("weekly-competitor-brief", evidence=lambda: {"brief": len(out.raw) > 500}) as run:
  ...
  dict(slug="autogen", name="AutoGen", group="Agent frameworks",
    title="AutoGen scheduled agent monitoring: conversations that never terminate, cost per run | RunVouch",
    desc="Heartbeat monitoring for scheduled AutoGen teams: multi-agent chats can run to max_turns without producing the result. RunVouch caps the run (BUDGET_RUN), detects loops (RETRY_STORM) and checks the output (NO_EVIDENCE).",
...
asyncio.run(main())'''),
  ...
  dict(slug="openai-agents-sdk", name="OpenAI Agents SDK", group="Agent frameworks",
    title="OpenAI Agents SDK scheduled run monitoring: max_turns, cost, evidence | RunVouch",
    desc="Heartbeat monitoring for scheduled OpenAI Agents SDK runs: wrap Runner.run() for a per-run cost cap (BUDGET_RUN), retry-storm detection on tool calls and a check that the agent produced its output (NO_EVIDENCE).",
...
asyncio.run(main())'''),
  ...
  dict(slug="pydantic-ai", name="Pydantic AI and smolagents", group="Agent frameworks",
    title="Pydantic AI and smolagents scheduled run monitoring | RunVouch",
    desc="Heartbeat monitoring for a nightly Pydantic AI or smolagents script: these libraries have no scheduler and no budget. Wrapping the script with rv run gives it MISSED, FAILED, NO_EVIDENCE, DRIFT and cost caps.",
...
open("/out/digest.md", "w").write(result.output)'''),
  ...
  dict(slug="dify-flowise", name="Dify and Flowise", group="Agent frameworks",
    title="Dify and Flowise scheduled workflow monitoring: the caller that stops quietly | RunVouch",
    desc="Heartbeat monitoring for Dify and Flowise workflows called from cron: wrap the caller with rv run so a stopped scheduler (MISSED) or an empty answer (NO_EVIDENCE) gets an alert, and pass token usage for the cost cap.",
...
json.dump(r.json(), open("/out/brief.json", "w"))'''),
  ...
  dict(slug="ollama-local-llm", name="Ollama and local LLM batch jobs", group="Agent frameworks",
    title="Ollama nightly batch job monitoring: stalls, drift, empty output | RunVouch",
    desc="Heartbeat monitoring for nightly Ollama and local LLM batch jobs: local models cost no API money, which is why nobody notices when the job stalls on a swapped-out model or writes an empty file. rv run gives it STALLED, DRIFT and NO_EVIDENCE.",
...
0 1 * * * rv run nightly-classify --evidence-file /data/labels.jsonl -- python classify.py --model llama3.1'''),
  ...
  dict(slug="n8n", name="n8n (community node)", group="No-code automation",
    ...
    desc="Heartbeat monitoring for n8n workflows on a Schedule Trigger with the n8n-nodes-runvouch community node (or two HTTP Request nodes): MISSED when the workflow stops running, NO_EVIDENCE when it ran and found nothing, cost caps per execution.",
...
POST https://api.runvouch.com/v1/runs/end     {"run_id":"...","status":"ok","evidence":{"leads_fetched":true}}'''),
  ...
  dict(slug="openclaw", name="OpenClaw (verified-run skill)", group="Agent frameworks",
    title="OpenClaw scheduled task monitoring with the verified-run skill: start, task, end with an evidence file | RunVouch",
    desc="Heartbeat monitoring for OpenClaw scheduled tasks: the verified-run skill wraps a task in a RunVouch run and reports the output file as evidence, so MISSED, FAILED and empty runs alert you. For the automations CLI and the gateway heartbeat, see the OpenClaw cron page.",
    h1="OpenClaw scheduled task monitoring", mode="wrap",
...
export RUNVOUCH_EVIDENCE='{"draft":{"type":"file","path":"out/CHANGELOG-draft.md"}}\''''),
  ...
  dict(slug="openclaw-cron", name="OpenClaw automations and heartbeat", group="Schedulers",
    ...
    where="Two layers. A host cron line wraps a health probe of the gateway so the whole instance has a heartbeat. Per job, the <a href=\"/openclaw/runvouch/SKILL.md\">RunVouch skill</a> makes the agent report start, tool calls and end with evidence; for a task that runs a script, the <a href=\"/integrations/openclaw\">verified-run skill</a> wraps it with the output file as evidence.",
