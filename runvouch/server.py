...
function copyping(u){navigator.clipboard.writeText(u).then(()=>alert('Ping URL copied. Append /start, /fail or the exit code. Anything that can call a URL can use it; it reports this one agent and nothing else.'),()=>prompt('Ping URL',u))}
...
s+='<h2>Agents</h2><table><tr><th>agent</th><th>state</th><th>last run</th><th>status</th><th>evidence</th><th>cost 24h</th><th>alerts</th><th>proof</th><th>ping url</th></tr>';
for(const a of ag.sort((x,y)=>(x.state==='ok')-(y.state==='ok'))){const l=a.last_run;s+=`<tr><td>${esc(a.name)}</td><td><span class="pill ${a.state}">${a.state}</span></td><td>${l?new Date(l.started*1000).toLocaleString():'-'}</td><td>${l?l.status:'-'}</td><td>${l?(l.evidence_ok===null?'-':l.evidence_ok?'✓':'✗'):'-'}</td><td>$${(a.cost_24h||0).toFixed(3)}</td><td>${a.open_alerts}</td><td>${l&&l.ended?`<a href="#" onclick="proof('${l.id}');return false">proof</a>`:'-'}</td><td>${a.ping_url?`<button onclick="copyping('${a.ping_url}')" title="report this agent from anything that can call a URL">copy</button>`:'-'}</td></tr>`}
